(function(){
  ...
  function installVoiceCollapse() {
    ...
    style.textContent = [
      ...
      '@media (max-width:760px){#pbrVoiceAgent{right:10px!important;bottom:92px!important;max-width:calc(100vw - 20px);}body.pbr-voice-collapsed #pbrVoiceAgent,body.pbr-voice-collapsed elevenlabs-convai{transform:translateY(calc(100% - 58px)) scale(.84);}}',
      ...
      '@media (max-width:760px){#pbrVoiceLauncher{right:14px;bottom:94px;width:58px;height:58px;}.pbrVoiceOrb{width:40px;height:40px;}}'
    ].join('\n');
    ...
    function collapseForScroll() {
      ...
      setCollapsed(mobileViewport.matches || window.scrollY > 24);
    }
    ...
    window.addEventListener('resize', function() {
      if (!expandedByUser) setCollapsed(mobileViewport.matches || window.scrollY > 24);
    });
